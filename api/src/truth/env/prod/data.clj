...
(def data
  (if (= (s/conform ::sh/claims shorthand-claims) :clojure.spec.alpha/invalid)
    (do
      (println "claims did not conform to spec"
               nil))
    (concat users books publications sources claims)))

(s/explain-str ::sh/claims shorthand-claims)
